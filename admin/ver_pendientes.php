...
if(isset ($_GET['id'])){
    $area = $_GET['id'];
    if($area == "todos"){
    $sql_select =  mysqli_query($con,"SELECT `id_pendientes`, `status`, `titulo`, `descripcion`, `area`, `fecha` FROM `pendientes` WHERE `status` = 0 AND `razon` IS NULL");
    $aa=0;
    }else{
    $sql_select =  mysqli_query($con,"SELECT `id_pendientes`, `status`, `titulo`, `descripcion`, `area`, `fecha` FROM `pendientes` WHERE `status` = 0 AND `area` = '$area' AND `razon` IS NULL");
    $aa=$area;
    }
}else{
    $aa=0;
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            ...
            <div class="row">
                ...
                <?php
                    require("modales/eliminar.php");
                    require("modales/editar.php");
