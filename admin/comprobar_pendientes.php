...
if(isset ($_GET['id'])){
    $area = $_GET['id'];
    if($area == "todos"){
    $sql_select =  mysqli_query($con,"SELECT `id_pendientes`, `status`, `titulo`, `descripcion`, `area`, `fecha`, `razon`  FROM `pendientes` WHERE `status` = 2");
    }else{
    $sql_select =  mysqli_query($con,"SELECT `id_pendientes`, `status`, `titulo`, `descripcion`, `area`,  `fecha`, `razon` FROM `pendientes` WHERE `status` = 2 && `area` = '$area'");
    }
}else{
    $area = "nada";
    $sql_select =  mysqli_query($con,"SELECT `id_pendientes`, `status`, `titulo`, `descripcion`, `area`, `fecha`, `razon`  FROM `pendientes` WHERE `status` = 2");
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            ...
            <div class="row">
                    ...
                    <div class="card">
                        <form method="POST" action="">
                            <div class="card-body">
                                ...
                                <?php }?>
                                <button type="button" data-target="#modal-completado_<?php echo $data['id_pendientes']; ?>" data-toggle="modal" class="btn btn-outline-primary btn-icon-text"><i class="mdi mdi-file-check btn-icon-prepend"></i> ACEPTAR </button>
                                <button type="button" data-target="#modal-rechazar_<?php echo $data['id_pendientes']; ?>" data-toggle="modal" class="btn btn-outline-danger btn-icon-text"><i class="mdi mdi-delete-forever btn-icon-prepend"></i> RECHAZAR </button>
                ...
                <?php
                    require("modales/rechazar.php");
                    require("modales/completado_comprobado.php");
