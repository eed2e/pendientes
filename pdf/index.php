...
if(isset ($_GET['id'])){
  $aa = $_GET['id'];
  if($aa == "todos" || $aa == "nada"  ){
    $sql_select =  mysqli_query($con,"SELECT `id_pendientes`, `status`, `titulo`, `descripcion`, `area`, `fecha` FROM `pendientes` ");
    $aa = "TODOS!!!!!!";
  }else{
  $sql_select =  mysqli_query($con,"SELECT `id_pendientes`, `status`, `titulo`, `descripcion`, `area`, `fecha` FROM `pendientes` WHERE `area` = '$aa' ");
  }
}else{
  $sql_select =  mysqli_query($con,"SELECT `id_pendientes`, `status`, `titulo`, `descripcion`, `area`, `fecha` FROM `pendientes` ");
  $aa = "TODOS!!!!!!";
